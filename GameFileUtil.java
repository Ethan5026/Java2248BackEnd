...
public class GameFileUtil {
	...
	public static void save(String filePath, ConnectGame game) {
		try {
			...
			//writes width, height, minimum tile level, max tile level, and score
			String line = "";
			line += grid.getWidth() + " " + grid.getHeight() + " " + game.getMinTileLevel() + " " + game.getMaxTileLevel() + " " + (int) game.getScore() + '\n';
			writer.write(line);
			line="";
			Tile tile;
			...
			for(int x = 0; x < grid.getHeight(); x++) {
				...
				for(int y = 0; y < grid.getWidth(); y++) {
					...
					//writes the tile to the string and adds space or new line appropriately 		
					if(y == grid.getWidth() - 1) {
						line += tile.getLevel() + "";
						if(x != grid.getHeight() - 1) {
							line += "" + '\n';
						}
				...
				//write string to the file and clear the string
				writer.write(line);
				//writer.newLine();
				writer.flush();
				line = "";
